...
function createTestWater(group)
{
	var path = new CustomSinCurve( 2 );
  var geometry = new THREE.TubeGeometry( path, 50, curveRadius, 30, false );
  ...
  var waterMaterial = new THREE.ShaderMaterial( {
		uniforms: {
			shallowColor: {value: new THREE.Vector4(0.1, 0.94, 0.999, 0.725)},
			deepColor: {value: new THREE.Vector3(0.086, 0.407, 1, 0.749)},
			textureWater: { type: "t", value: new THREE.TextureLoader().load( "images/water.jpg" ) },
			textureNoise: { type: "t", value: new THREE.TextureLoader().load( "images/PerlinNoise.png" ) },
			normalWater: { type: "t", value: new THREE.TextureLoader().load( "images/waterNormal.jpg" ) },
			time: {value: 0},
